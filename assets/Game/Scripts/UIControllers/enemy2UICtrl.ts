...
export class enemy2UICtrl extends UIComponent {
    ...
    onAnimationFinished(event:Animation.EventType,state:AnimationState){
        if(event==Animation.EventType.FINISHED){
            if(state.name=="enemy2Die"){
                SoundManager.Instance.PlaySound("enemy2_down");
                EventManager.Instance.Emit("updateScore",{score:this.data.Score});
                 this.node.destroy();
            }
            else if(state.name=="enemy2Hit"){
                 this.node.getComponent(Animation).play("enemy2Normal");
            }
        }
    }
    ...
    onBeginContact (selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) {
        if(otherCollider.tag==Collider2DTag.Bullet){
            ...
            if(this.data.Hp<=0){
                ...
                animation.play("enemy2Die");
            }
            else{
                animation.play("enemy2Hit");
            }
        }
