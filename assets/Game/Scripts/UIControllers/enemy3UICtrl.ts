...
export class enemy3UICtrl extends UIComponent {
    ...
    onAnimationFinished(event:Animation.EventType,state:AnimationState){
        if(event==Animation.EventType.FINISHED){
            if(state.name=="enemy3Die"){
                SoundManager.Instance.PlaySound("enemy3_down");
                EventManager.Instance.Emit("updateScore",{score:this.data.Score});
                this.node.destroy();
            }
            else if(state.name=="enemy3Hit"){
                 this.node.getComponent(Animation).play("enemy3Normal");
            }
        }
    }
    ...
    onBeginContact (selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) {
        if(otherCollider.tag==Collider2DTag.Bullet){
            ...
            let animation=this.node.getComponent(Animation);
            if(this.data.Hp<=0){
                animation.play("enemy3Die");
            }
            else{
                animation.play("enemy3Hit");
            }
        }
